...
- Paste Stack использует отдельную reusable borderless `.nonactivatingPanel` configuration с теми же safe-area frame и mask transitions. Она не вызывает app activation/`makeKey`, не получает History outside-click/resign-key hooks и до S033 остаётся раскрытой до Cancel, global Escape или auto-finish. Целевой compact/expanded lifecycle определён в разделе S033 ниже. Отдельный movable frame, window drag region и saved-origin restore удаляются из active path;
...
## S033: компактная presentation Paste Stack

Реализован и принят пользователем 2026-09-15. D-043 меняет только Stack presentation. `PanelController`, `TopNotchHistoryGeometry`, `TopNotchPanelLifecycle`, `PasteStackPresentationModel` и `PasteStackPanelView` являются точками интеграции; session/input/capture/paste owners остаются прежними.

- Compact использует один центральный блок, ограниченный шириной экрана. На MacBook его ширина покрывает camera gap и по 64 pt с каждой стороны. Контент отступает от auxiliary-area boundaries на 8 pt; фон непрерывен под вырезом. Верх блока совпадает с `frame.maxY`, высота равна `safeAreaInsets.top` или высоте строки меню. Общая mask и hit region покрывают весь блок без центрального разрыва. Размер NSPanel совпадает с размером блока. Компактная mask использует тот же контур с обратными верхними скруглениями, что expanded panel, с радиусами 8/10 pt. SwiftUI content привязан к topLeading перед применением camera-safe offsets; квадратные карточки не растягиваются по ширине.
- Expanded layout использует размер History с её bounded/clamped geometry, но отдельный Stack lifecycle. History constants и поведение не меняются глобально ради Stack.
- Число NSPanel является внутренним решением после probe. Если используются несколько окон, они имеют одного presentation owner, session и generation token. В compact состоянии отсутствует большая прозрачная input surface под полосой; visual mask сам по себе не считается доказательством click-through. Hit testing проверяется реальными кликами.
- Hover tracking ограничивается реальными видимыми участками; вырез и пустое пространство не являются hover targets. Переходы запускаются без hover/collapse delay, отменяемы и проверяют session/generation. Drag/menu/accessibility holds не меняют StackSession. Observe updates не запускают polling, payload I/O или перезапуск анимации.
- Stack остаётся nonactivating/non-key, без `NSApp.activate`, `makeKey` и History outside-click hooks. Для VoiceOver доступны явные expand/collapse actions без зависимости от положения мыши.
- Screen disconnect, safe-area change и Space transitions пересчитывают placement и hit regions во всех фазах, включая reveal/dismiss. Этот путь вошёл в ручную приёмку S033; BL-006 остаётся отдельным принятым ограничением S030.
- Runtime probe не должен читать содержимое чужих меню или добавлять permissions. Не использовать private API или повышать window level без проверки системных меню. Auxiliary geometry не сообщает занятость menu/status items.

Проверка platform sources 2026-09-14: Apple [safeAreaInsets](https://developer.apple.com/documentation/appkit/nsscreen/safeareainsets) и [auxiliaryTopLeftArea](https://developer.apple.com/documentation/appkit/nsscreen/auxiliarytopleftarea-uglc) подтверждают camera-safe geometry и global coordinates боковой области. [auxiliaryTopRightArea](https://developer.apple.com/documentation/appkit/nsscreen/auxiliarytoprightarea-gr2n) задаёт правую область. Эти API уже используются проектом; minimum macOS, entitlements, Accessibility и distribution не меняются. Пользователь подтвердил overlay в menu band и native hit testing в ручной приёмке 2026-09-15.

## S034: typed payload и session ownership в Paste Stack

Контракт реализован локально 2026-09-15 в существующих History/Stack owners; installed-app, signing и update gates остаются отдельной проверкой. Этот раздел заменяет text-only routing/snapshot ограничения выше для S034; S031 History сохраняет собственные rich/plain actions.

- `StackCollectionCaptureCoordinator` публикует supported persisted descriptor после durable History commit и успешного получения session lease. Проверки session UUID/start watermark сохраняются до и после async work. `.images` добавляется в Stack, `.references`/`.mixed` остаются History-only. Rich canonical string перестаёт быть единственным paste payload.
- `StackOccurrence` сохраняет identity/order/state и bounded presentation metadata, а immutable payload handle различает text/rich/image и ordered items. Raw assets не входят в session/UI arrays. Text preview не используется как payload. Shared materializer/writer History переиспользуется через явный контракт, без вызова History activation/conceal executor из Stack.
- `StackSequentialPasteExecutor` сохраняет синхронную reservation на event-tap boundary, затем получает payload асинхронно вне main actor. Перед write и tagged dispatch проверяет session, reservation, revocation и trust. Repeat при reservation потребляется без второго paste. Только успешный dispatch отмечает exact occurrence used.
- Writer возвращает exact final changeCount, который немедленно регистрируется как self-write даже если session отменяется после write. Нельзя отправлять stale или частично материализованный payload. Сохраняется fail-closed проверка внешней смены pasteboard перед dispatch; если текущий общий writer её не обеспечивает, добавить проверку на Stack path с regression test.
- Session lease реализуется внутри существующего storage owner по opaque asset/occurrence IDs, без нового persistence dependency. Automatic expiry может убрать History metadata, но held descriptor содержит достаточно неизменяемой информации для materialization; leased originals остаются quota-accounted до release. Delete transaction инвалидирует descriptor/lease до удаления, Clear All инвалидирует session до очистки. Deferred materialization и writer сериализуются с revocation на общей boundary, чтобы check-then-write race не восстановил удалённый payload.
- Explicit Delete очищает и text snapshot соответствующей Stack occurrence, сохраняя unavailable slot; Clear All отменяет Stack. Это изменение прежнего text snapshot lifecycle должно быть отражено в deletion UI и tests. Не создавать hidden копию вручную удалённого содержимого ради успешной вставки.
- Cancel/finish освобождают session leases, thumbnails и payload references. После crash leases не восстанавливаются; startup reconciliation удаляет безопасно распознанные unowned assets. Used assets удерживаются до конца session ради Reactivate. Global image/rich ceilings и bounded capture admission не повышаются; нет безлимитного второго asset store.
- Plain text/RTF/HTML и images используют уже принятые allowlists и limits S024/S031. Mixed source normalization, metadata schema и target capabilities не расширяются автоматически. Если для ownership требуется schema migration, сохранить прежние UUID/activityAt/favorites и добавить проверку old-store update до release.

Зависимости: реализация и приёмка S033, существующие S024/S031 payload contracts. Открытый signed-update gate S031 сохраняется как delivery dependency, а не как доказательство отсутствия пригодного materializer code. Новые системные API, permissions, min macOS и distribution mechanisms не требуются; compatibility source/target проверяется матрицей S034.


## S035: Finder Cut adapter

D-046 и S035 вводят отдельную process-local Cut session с generation, validated reference descriptors, clipboard changeCount и фазами preparing/ready/handedOff/blockedByPasteStack/failed. History file references S025 используются как опыт typed parsing, а не как источник move intent. В первой реализации допускаются только локальные regular files; source bytes не принадлежат Qipli.

Реализация использует существующий CGEventTapAdapter и tagged commands, контекст Finder через публичный Accessibility API, native Finder ⌘C при подготовке и ⌥⌘V при вставке. Подготовительный native clipboard сохраняется без reconstruction/private-type invention. AX read не выполняется синхронно в event tap; freshness и focus перепроверяются перед dispatch. При неопределённости ввод проходит без подмены.

Input arbitration допускает только одного владельца обычного ⌘V: Stack либо Cut. Cut действует исключительно для допущенного Finder destination с актуальным session/clipboard и не перехватывает текстовую вставку. Если clipboard устарел до dispatch, Cut инвалидируется; обычный ⌘V replay-ится только при неизменном destination context, а при смене focus/app событие безопасно отменяется без replay в новый target. Остальные случаи сохраняют original event. Native Finder owns move/conflicts/permissions; Qipli не вызывает source FileManager copy/remove. Отмена intent не отменяет переданную Finder операцию.

Finder Cut имеет отдельный `FinderCutPresentationModel` с состояниями hidden/compact/dismissing. Расширенного view, hover/click expansion и accessibility expansion action нет; `PasteStackPresentationModel` остаётся владельцем раскрытия только Stack. Существующая camera-safe geometry сохраняется: state symbol слева, cancel button справа в верхней полосе; нижняя строка показывает имя/количество либо компактное status/error message. Tooltip и VoiceOver дают полный statusDetail. После handedOff крестик закрывает только notice, не отменяет уже переданную Finder операцию. Generation guard отменяет устаревшее завершение fade-out при новом Cut; смена display geometry не блокирует завершение dismissal.

Критический probe описан в S035: file-selection admission, bounded prepare-copy correlation и monitor suppression, destination focus, одноразовый dispatch и наблюдаемость результата. Synthetic probe 2026-09-21 подтвердил tagged ⌘C → ⌥⌘V для локального файла и отсутствие собственного copy/delete в Qipli; прямой Finder ⌘X не дал отдельного move-сигнала, а result/progress API не обнаружен. Same-URL provenance остаётся ограничением публичных pasteboard/Accessibility API; защита использует exact next changeCount, 1 s window, актуальный Finder selection и fail-closed поведение вне Finder. Поэтому реализация сообщает только dispatch в Finder, не успех операции. Новые permissions/minimum macOS/distribution не добавляются.

Apple sources проверены 2026-09-14: [Mac keyboard shortcuts](https://support.apple.com/en-mide/102650) документирует ⌥⌘V для перемещения clipboard files; [CGEventTapCreate](https://developer.apple.com/documentation/coregraphics/cgevent/tapcreate(tap:place:options:eventsofinterest:callback:userinfo:)) описывает event tap. Synthetic probe выполнен 2026-09-21; installed-app live acceptance ещё не выполнялась.

## S036: последние записи History в status menu

Новая точка входа использует существующие History storage и paste executor. Схема данных, permissions, event tap, deployment и capture formats не меняются.

- `ApplicationShell.configureStatusItem()` устанавливает `RecentHistoryMenuController` как native menu delegate. Отдельный `HistoryViewModel.recentState` получает bounded projection через `HistoryService.recentDescriptors()`, независимо от Search/Favorites/selection. Store возвращает страницы по пять display descriptors; projection содержит максимум пять renderable occurrences с UUID в обычном activity order.
- Поддерживать projection после завершённых capture, promotion, delete, clear и retention операций через существующий последовательный persistence pipeline. При открытии запросить асинхронную актуализацию, включая pending capture drain. Не загружать весь store ради пяти строк и не выполнять sync I/O в menu callbacks. До первого snapshot допустим disabled loading row; ошибку чтения отличать от пустой History.
- На одно открытие фиксировать mapping строк к UUID. Обновления не должны переназначать уже подсвеченный пункт на другую occurrence. Новый порядок показывать при следующем открытии; deleted/revoked ID повторно валидировать при выборе.
- До menu focus capture сохранить конкретное внешнее target application существующим capture adapter. Выбор после закрытия native menu передаёт target и UUID в общий History paste coordinator/executor. Не захватывать target заново из приложения, случайно ставшего активным позже. Не использовать stale target прежнего открытия History.
- Materialize только выбранный payload с существующими typed writer, rich default, exact self-write changeCount и tagged dispatch. Общая transaction guard исключает параллельные вставки из панели/меню. Activity promotion следует существующему результату executor; dispatch не доказывает принятие данных target app.
- Ошибка target/permission/payload не вызывает вставку в другое приложение или молчаливую деградацию. Показать существующий non-payload notice или History failure UI с сохранённым UUID; это допустимое исключение из пути без открытия панели.
- На первом срезе при active Stack строки видимы, но недоступны, с кратким объяснением необходимости завершить Stack. Это предложенное ограничение планирования сохраняет BR-028 и исключает изменение Stack queue/used-state. S035 не является зависимостью; если Cut будет реализован, его invalidation при History paste применяется и к меню.
- Native menu items используют одну короткую строку, локальный type icon и доступное имя/состояние. Text/rich previews нормализуют переносы и обрезаются только для отображения. Image/file/video/URL используют существующую display projection и fallback типа/количества. Thumbnails, HTML rendering, remote metadata и новые numeric shortcuts не входят.
