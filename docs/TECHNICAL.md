...
2. По `Enter` выбранный текст становится внутренней записью в system pasteboard.
3. Панель закрывается, прежнее приложение активируется, затем отправляется синтетический `⌘V`.
4. UI сообщает только об отправке команды; реальное принятие текста сторонним приложением наблюдать надёжно нельзя.
5. При отказе активации или отсутствии разрешения команда не считается успешной, запись не удаляется.
