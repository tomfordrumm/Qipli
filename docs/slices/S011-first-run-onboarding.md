...
- Accessibility prompt вызывается только кнопкой `Allow Access`. Deny, закрытие System Settings или Skip не блокируют flow; UI показывает фактический state.
...
- [x] Accessibility запрашивается только после явной кнопки; grant, deny, Skip и возврат из System Settings оставляют flow управляемым и показывают фактическое состояние.
