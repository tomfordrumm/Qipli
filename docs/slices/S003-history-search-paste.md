...
Автоматические проверки завершены. Для `done` потребуется ручная проверка, что `⌘⇧V` не выполняет action в target app до показа History, всегда force-activates Qipli и делает History key с autofocus без click после hotkey/reopen, Up/Down/Enter/Esc проходят без SwiftUI console warning `Publishing changes from within view updates`, read-only entries, single-click selection, double-click exact paste и Delete without selection/paste, а `Enter` реально вставляет в TextEdit/browser/editor (без оставшейся History и без ручного `⌘V`), immediate/delayed unavailable target оставляет History visible с retryable error без duplicate paste, `Esc` focus return, permission-denied UI и read-only/secure field without false success.
