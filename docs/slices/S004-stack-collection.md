...
- Только копирования с хотя бы одним непробельным символом, произошедшие после старта, добавляются в session.
...
- [x] Изменения без текстового представления, whitespace-only изменения и self-writes Qipli не добавляются; игнорирование whitespace-only не считается ошибкой, а ошибка сохранения истории не создаёт «осиротевший» элемент только в стеке.
