...
- Точный следующий шаг: по команде пользователя начать [`S004 — Сбор и визуальная панель Paste Stack`](slices/S004-stack-collection.md); до начала статус остаётся `planned`.
...
- S001–S003 имеют статус `done`; остальные срезы остаются `planned`.
