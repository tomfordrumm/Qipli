...
- `docs/RELEASING.md` — порядок подготовки, публикации и восстановления релиза.
...
### Ветки и публикация релизов

- Рабочие ветки называются `feature/<описание>`, включая исправления и документацию. Релизные ветки называются только `release/X.Y.Z`, теги только `vX.Y.Z`. Эти правила переопределяют стандартный префикс Codex `codex/`.
- Перед релизной работой прочитайте [docs/RELEASING.md](docs/RELEASING.md) и актуальные gates в `docs/STATE.md`. Новую релизную ветку создавайте от свежего `origin/main`, когда выбранные изменения уже слиты через PR. В ней меняются только версия, build number и release notes.
- Подготовку версии оформляйте одним PR `Release X.Y.Z` в `main`. Тег ставьте на выбранный коммит `main` после merge и успешного unsigned CI именно этого SHA. Публикация выполняется существующим protected release workflow.
- «Подготовь релиз» разрешает локальную подготовку и проверки. «Создай релизный PR» дополнительно разрешает commit, push и PR. «Опубликуй релиз X.Y.Z» разрешает необходимые подготовку, commit, push, PR, merge, tag, запуск workflow и проверку публикации без повторных подтверждений каждого шага. Protected Environment approval и обязательные gates сохраняются. Отдельный запрос на commit или push не разрешает публикацию.
- В отчёте укажите версию/build, SHA/тег, PR/workflow, результат проверки DMG/ZIP и Sparkle, оставшиеся ручные gates. Публикация и установленное обновление имеют отдельные результаты проверки. Опубликованные теги и артефакты неизменяемы; исправления выпускаются новой версией.
- Переходное исключение: уже существующая `codex/release-1.0.11` доставляется через PR в `main` без переименования и пересоздания. Новая схема применяется к следующим веткам; старые ветки автоматически не переименовываются.
